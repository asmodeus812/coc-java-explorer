...
export class Settings {
    public static initialize(context: ExtensionContext): void {
        context.subscriptions.push(
            workspace.onDidChangeConfiguration((e: ConfigurationChangeEvent) => {
                if (
                    (e.affectsConfiguration("java.dependency.syncWithFolderExplorer") && Settings.syncWithFolderExplorer()) ||
                    ...
                    DependencyExplorer.getInstance(context).dataProvider.setRefreshDebounceFunc(Settings.refreshDelay());
                }
            })
        );
        ...
        context.subscriptions.push(commands.registerCommand(Commands.VIEW_PACKAGE_LINKWITHFOLDER, Settings.linkWithFolderCommand));

        context.subscriptions.push(commands.registerCommand(Commands.VIEW_PACKAGE_UNLINKWITHFOLDER, Settings.unlinkWithFolderCommand));

        context.subscriptions.push(
            commands.registerCommand(Commands.VIEW_PACKAGE_CHANGETOFLATPACKAGEVIEW, Settings.changeToFlatPackageView)
        );

        context.subscriptions.push(
            commands.registerCommand(Commands.VIEW_PACKAGE_CHANGETOHIERARCHICALPACKAGEVIEW, Settings.changeToHierarchicalPackageView)
        );
    }
    ...
    public static switchNonJavaResourceFilter(enabled: boolean): void {
        workspace.getConfiguration("java.project.explorer").update("showNonJavaResources", enabled, ConfigurationTarget.Workspace);
    }
    ...
    public static getExportJarTargetPath(): string {
        return workspace
            .getConfiguration("java.project.exportJar")
            .get<string>("targetPath", "${workspaceFolder}/${workspaceFolderBasename}.jar");
    }
